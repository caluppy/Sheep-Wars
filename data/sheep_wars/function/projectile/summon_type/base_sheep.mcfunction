...
$execute at @s run summon minecraft:sheep ^ ^ ^1 {attributes:[{id:"minecraft:fall_damage_multiplier",base:0}],Rotation:$(Rotation),data:{"playerUUID":$(UUID)},OnGround:0b,DeathLootTable:"sheep_wars:entities/death_projectile/base_sheep",Tags:["sheep_wars.motion.linear","sheep_wars.explosion.base","sheep_wars.projectile.new"]}
