...
$execute at @s anchored eyes run summon minecraft:sheep ^ ^ ^ {Color:2,attributes:[{id:"movement_speed",base:0},{id:"minecraft:fall_damage_multiplier",base:0},{id:"minecraft:air_drag_modifier",base:0}],Rotation:$(Rotation),data:{"owner":$(UUID),"playerUUID":$(UUID)},OnGround:0b,DeathLootTable:"minecraft:empty",Tags:["sheep_wars.type.magentalamb","sheep_wars.motion.linear","sheep_wars.projectile.new"]}
playsound minecraft:entity.shulker.shoot master @a[distance=..100] ~ ~ ~ 1.3 0.7 0.5
clear @s magenta_dye[custom_data={right_click:true}] 1
