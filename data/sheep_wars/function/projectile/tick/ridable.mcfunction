...
#tp
$execute rotated as @p[scores={player_UUID_0=$(playerUUID)}] at @s run tp @n[predicate=sheep_wars:projectile/clean/riding_motion_marker,scores={player_UUID_0=$(playerUUID),entity_UUID_0=$(entityUUID)}] ^ ^ ^0.5

#final pos -> vel -> motion
$execute store result score @s position.x run data get entity @n[predicate=sheep_wars:projectile/clean/riding_motion_marker,scores={player_UUID_0=$(playerUUID),entity_UUID_0=$(entityUUID)}] Pos[0] 1000
$execute store result score @s position.y run data get entity @n[predicate=sheep_wars:projectile/clean/riding_motion_marker,scores={player_UUID_0=$(playerUUID),entity_UUID_0=$(entityUUID)}] Pos[1] 1000
$execute store result score @s position.z run data get entity @n[predicate=sheep_wars:projectile/clean/riding_motion_marker,scores={player_UUID_0=$(playerUUID),entity_UUID_0=$(entityUUID)}] Pos[2] 1000
...
scoreboard players add @s[predicate=sheep_wars:projectile/summon_type/ride_carpet_squid] new_projectile.cooldown 1
execute if score @s[predicate=sheep_wars:projectile/summon_type/ride_carpet_squid] new_projectile.cooldown matches 40 as @s at @s run function sheep_wars:projectile/schedule/carpet_bomb with entity @s data
